...
# Help function
show_help() {
    echo -e "${BLUE}DiamondLinks Agent Runner${NC}"
    echo ""
    echo "Usage: ./run-agent.sh [agent] [model] [loops]"
    echo ""
    echo "Agents:"
    echo "  redesign  - Visual redesign: screenshot localhost:3004, analyze, and improve UI/design (recommended: opus, 20 loops)"
    echo ""
    echo "Models:"
    ...
    echo ""
    echo "Prerequisites:"
    echo "  - Dev server running on port 3004: npm run dev"
...
case "${1:-help}" in
    redesign)
        run_agent "DiamondLinks Redesign Agent" "$AGENTS_DIR/visual-enhancement-agent" "💎" "$MODEL" "${LOOPS:-15}"
        ;;
